    ...
    <section className={`${styles.wrapper} w-full px-6 md:px-12`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {slot.testimonial_notes.map((note, index) => (
            <div key={index} className={`${styles.card} rounded-xl p-6`}>
              <p className={`${styles.body} text-sm leading-relaxed mb-6`}>
                "{note.quote}"
              </p>
              <div className="flex items-center justify-between">
                <p className={`${styles.heading} text-sm`}>{note.author_name}</p>
                <p className={`${styles.subheading} text-xs`}>{note.author_date}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center">
          <button className={`${styles.action} px-6 py-2 text-sm`}>
            {slot.section_cta}
          </button>
        </div>
