// Hero_06.meta.ts
import { type Hero_06Meta } from './Hero_06.types';

const meta: Hero_06Meta = {
  version: "1.1",
  ...
  category: "hero",
  intent: "convert",
  layout_structure: "split-video-left", // Key difference: video on left, content on right

  // === Tokens ===
  tokens: {
    // Layout Structure - Video Left, Content Right
    layout: {
      ...
      contentColumn: "flex flex-col justify-center order-1 lg:order-2 min-h-[50vh] lg:min-h-0", // Content on right
  ...
  layoutContract: {
    ...
    breakpoints: {
      mobile: "single-col",
      tablet: "split-video-left",
      desktop: "split-video-left"
    }
  },
  ...
  accessibility: {
    ...
    screenReaderHints: {
      ...
      section: "Main hero content with video on left"
    }
  },
  ...
  // === Semantic Profile ===
  semanticProfile: {
    primaryConcepts: ["conversion", "introduction", "value-proposition", "trust-building", "video-storytelling"],
    contentTypes: ["hero-messaging", "cta-copy", "brand-positioning", "video-content"],
    embeddingWeights: { visual: 0.5, copy: 0.3, interaction: 0.2 }, // Higher visual weight for video
    contextualRelevance: ["landing-page-entry", "product-introduction", "conversion-funnel-top", "video-marketing"]
  },

  // === Figma Export ===
  figma: {
    autoLayout: true,
    figmaComponentName: "Hero/06 - Split with Video Left",
    componentSets: {
  ...
  code: {
    ...
    imports: [
      ...
      "import { type Hero_06Props, type ColorOverrides } from './Hero_06.types';",
      "import { validateHero06Props, sanitizeProps, formatValidationMessage } from './Hero_06.validators';",
      "import meta from './Hero_06.meta';"
    ],
    template: `const Hero_06: React.FC<Hero_06Props> = (props = {}) => {
    const { theme } = useTheme();
    const validation = validateHero06Props(props);
    ...
    return (
        <section
            ...
            role="banner"
            aria-label="Main hero content with video on left"
        >
  ...
  development: {
    ...
    documentation: {
      designNotes: "Primary conversion component for landing pages with integrated video player on left side. All styles are tokenized using native HTML elements. Fully independent with no component dependencies.",
      usageGuidelines: "Use native h1, p, button/a, video elements with complete token classes. Colors are fully customizable via props. Animations can be disabled for accessibility. No external component dependencies. Video player integrated directly on left side."
    }
  }
};
