...
describe('Hero_12 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_12.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_12.meta'));
    });
  ...
  it('should have proper center-bottom structure', () => {
    const { container } = render(
      <Hero_12
        title="Test Title"
        description="Test Description"
    ...
    // Check for center-bottom specific elements
