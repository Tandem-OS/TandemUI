...
describe('Hero_15 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_15.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => 
      imp.includes('tokens/') || imp.includes('design-system/') || (imp.includes('meta') && !imp.includes('./Hero_15.meta'))
    );
  ...
  it('should have proper video background hero structure with content positioning', () => {
    const { container } = render(
      <Hero_15
        title="Test Title"
        description="Test Description"
  ...
  it('should have correct layout structure for content left bottom, title right top', () => {
    const { container } = render(
      <Hero_15
        title="Test Title"
        description="Test Description"
