...
export function validateHero08Props(props: Hero_08Props): ValidationResult {
    ...
    if (props.videoSrc !== undefined) {
        ...
        } else if (!isValidVideoURL(props.videoSrc)) {
            errors.push({
                field: 'videoSrc',
    ...
    if (props.videoThumbnail !== undefined) {
        ...
        } else if (!isValidURL(props.videoThumbnail) && !props.videoThumbnail.startsWith('/')) {
            errors.push({
                field: 'videoThumbnail',
    ...
    // 7. Enhanced color validation with newsletter and video-specific colors
        ...
        if (typeof props.colors !== 'object' || props.colors === null) {
            ...
            const validateColor = (color: unknown, path: string, displayPath: string) => {
                ...
                if (typeof colorObj.light !== 'string' || !PATTERNS.hexColor.test(colorObj.light as string)) {
                    // Allow rgba colors for video overlay
                    if (path.includes('overlay') && (colorObj.light as string).startsWith('rgba(')) {
                        return; // Valid rgba color for overlay
                    }
                    errors.push({
                        field: `${path}.light`,
                        message: `Must be a valid hex color or rgba (got: "${colorObj.light}")`,
                        userMessage: `${displayPath} light color must be a hex code (e.g., #FFFFFF) or rgba value`
                    });
                }

                if (typeof colorObj.dark !== 'string' || (!PATTERNS.hexColor.test(colorObj.dark as string) && !(colorObj.dark as string).startsWith('rgba('))) {
                    errors.push({
                        field: `${path}.dark`,
                        message: `Must be a valid hex color or rgba (got: "${colorObj.dark}")`,
            ...
            const validateVideoColors = (video: unknown, path: string) => {
                ...
                // Validate overlay color (can be rgba)
                validateColor(videoObj.overlay, `${path}.overlay`, 'Video overlay');

                // Validate play button colors
...
export function sanitizeProps(props: Hero_08Props): Hero_08Props {
...
export function validateLayoutSafety(props: Hero_08Props): { safe: boolean; warnings: string[] } {
    const warnings: string[] = [];
