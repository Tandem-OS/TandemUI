...
// Main Hero_13 component
const Hero_13: React.FC<Hero_13Props> = (props = {}) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        if (!validation.valid || validation.warnings) {
            console.group('Hero_13 Validation');
    ...
    return (
        <section
            ...
            role="banner"
            aria-label={meta.accessibility.screenReaderHints?.section || "Main hero content with reverse responsive layout"}
            id={uniqueId}
        >
            {/* Background Image */}
            <div
            ...
            <div className={`${meta.tokens.layout.contentContainer} ${meta.tokens.spacing.sectionY}`}>
                <div className={meta.tokens.layout.grid}>
                    {/* Mobile: Heading Top / Desktop: Description Bottom (Left Column) */}
                    <div className={meta.tokens.layout.leftColumn}>
                        {/* Mobile Heading (visible on mobile only) */}
                        <MotionWrapper
                            className={meta.tokens.layout.mobileHeadingWrapper}
                            {...(animated ? getAnimationProps(fadeInUp, 0, 0.3, true) : {})}
                            data-testid="hero-mobile-heading"
                        >
                            <h1
                        ...
                        {/* Desktop Description (visible on desktop only) */}
                        <MotionWrapper
                            className={meta.tokens.layout.desktopDescriptionWrapper}
                            {...(animated ? getAnimationProps(fadeInUp, 0.1, 0, true) : {})}
                            data-testid="hero-desktop-description"
                        >
                            <p
                    ...
                    {/* Mobile: Description Bottom / Desktop: Heading Center (Right Column) */}
                    <div className={meta.tokens.layout.rightColumn}>
                        {/* Mobile Description (visible on mobile only) */}
                        <MotionWrapper
                            className={meta.tokens.layout.mobileDescriptionWrapper}
                            {...(animated ? getAnimationProps(fadeInUp, 0.1, 0, true) : {})}
                            data-testid="hero-mobile-description"
                        >
                            <p
                                className={meta.tokens.typography.body.complete}
                                style={{ color: 'var(--hero-desc)' }}
                        ...
                        {/* Desktop Heading & CTAs (visible on desktop only) */}
                        <MotionWrapper
                            className={meta.tokens.layout.desktopHeadingWrapper}
                            {...(animated ? getAnimationProps(fadeInUp, 0, 0.3, true) : {})}
                            data-testid="hero-desktop-heading"
                        >
                            <h1
                                className={meta.tokens.typography.heading.complete}
                                style={{ color: 'var(--hero-title)' }}
