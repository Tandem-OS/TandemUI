...
describe('Hero_13 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_13.tsx');
  ...
  it('should use meta.tokens throughout the component', () => {
    ...
    expect(tokenUsage).toBeGreaterThanOrEqual(12);
  });
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_13.meta'));
    });
  ...
  it('should only use styled-components for hover states', () => {
    ...
    expect(styledComponents).toBeLessThanOrEqual(2); // Only HoverButton and HoverLink
  });

  it('should have proper reverse swap structure', () => {
    const { container } = render(
      <Hero_13
        title="Test Title"
        description="Test Description"
    ...
    // Check for essential background structure
    ...
    expect(container.querySelector('[data-testid="hero-image"]')).toBeTruthy();
    
    // Check for reverse swap specific elements
    expect(container.querySelector('[data-testid="hero-mobile-heading"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="hero-desktop-description"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="hero-mobile-description"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="hero-desktop-heading"]')).toBeTruthy();
  });
});