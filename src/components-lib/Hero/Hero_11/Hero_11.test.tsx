...
describe('Hero_11 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_11.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_11.meta'));
    });
  ...
  it('should have proper fixed positioning structure', () => {
    ...
    // Check for fixed positioning specific elements
