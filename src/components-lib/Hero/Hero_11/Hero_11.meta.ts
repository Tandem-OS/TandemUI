// Hero_11.meta.ts
import { type Hero_11Meta } from './Hero_11.types';

const meta: Hero_11Meta = {
  version: "1.1",
  ...
  tokens: {
    ...
    layout: {
      ...
      headingCtaWrapper: "w-full space-y-md lg:space-y-lg",
  ...
  defaults: {
    ...
    // Default Colors for Fixed Positioning Hero
    colors: {
      overlay: {
  ...
  accessibility: {
    landmarks: ["banner"],
    ariaHints: ["h1 first", "background image decorative", "fixed content positioning"],
    colorContrastMin: 4.5,
    focusManagement: "linear",
    keyboardNavigation: true,
    screenReaderHints: {
      image: "decorative background",
      section: "Main hero content with fixed positioning layout",
  ...
  generation: {
    aiPromptHints: [
      "Fixed positioning background hero component",
      "Heading and CTAs positioned in left column",
      "Description positioned in right column",
      "Business and corporate focused",
      "Clear contrast needed for text readability"
    ],
    contentConstraints: {
      title: {
        tone: ["professional", "compelling", "confident"],
  ...
  // === Semantic Profile ===
  semanticProfile: {
    primaryConcepts: ["business-conversion", "fixed-positioning", "professional-messaging", "dual-column"],
    contentTypes: ["business-messaging", "corporate-copy", "conversion-focused"],
    embeddingWeights: { visual: 0.4, copy: 0.4, interaction: 0.2 },
    contextualRelevance: ["business-landing", "corporate-site", "service-introduction", "conversion-funnel"]
  },

  // === Figma Export ===
  figma: {
    autoLayout: true,
    figmaComponentName: "Hero/11 - Background Fixed Positioning",
    componentSets: {
  ...
  code: {
    ...
    imports: [
      ...
      "import { type Hero_11Props, type ColorOverrides } from './Hero_11.types';",
      "import { validateHero11Props, sanitizeProps, formatValidationMessage } from './Hero_11.validators';",
      "import meta from './Hero_11.meta';"
    ],
    template: `const Hero_11: React.FC<Hero_11Props> = (props = {}) => {
    const { theme } = useTheme();
    const uniqueId = useId();
    
    const validation = validateHero11Props(props);
  ...
  development: {
    storybook: {
      title: "Components/Hero/Hero_11",
      category: "Layout"
    },
    testing: {
      testIds: ["hero-section", "hero-background", "hero-overlay", "hero-title", "hero-description", "hero-buttons", "hero-left-column", "hero-right-column", "hero-heading-cta-wrapper", "hero-description-wrapper"],
      accessibilityTests: true
    },
    documentation: {
      designNotes: "Fixed positioning background hero component with heading/CTAs on left and description on right. All styles are tokenized using native HTML elements. Optimized for business and corporate use cases.",
      usageGuidelines: "Use native h1, p, button/a elements with complete token classes. Background image should be high-quality. Content positioning remains consistent across breakpoints. Ensure sufficient contrast for accessibility."
    }
  }
};
