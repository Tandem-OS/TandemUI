...
describe('Hero_24 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_24.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_24.meta'));
    });

    expect(invalidImports).toHaveLength(0);
  });

  // Newsletter-specific test - Simple addition
  it('should include newsletter form elements', () => {
    ...
    expect(getByRole('button', { name: /subscribe/i })).toBeInTheDocument();
  });
});