...
export function validateHero24Props(props: Hero_24Props): ValidationResult {
    ...
    // 7. Enhanced color validation with newsletter-specific colors
                ...
                if (newsletterObj.button && typeof newsletterObj.button === 'object') {
                    ...
                    if (buttonObj.hover !== undefined) {
                        if (typeof buttonObj.hover !== 'object' || buttonObj.hover === null) {
                            errors.push({
                                field: `${path}.button.hover`,
                                message: 'Button hover must be an object',
                                userMessage: `Newsletter button hover colors are invalid`
                            });
                        } else {
...
export function sanitizeProps(props: Hero_24Props): Hero_24Props {
...
export function validateLayoutSafety(props: Hero_24Props): { safe: boolean; warnings: string[] } {
    const warnings: string[] = [];
