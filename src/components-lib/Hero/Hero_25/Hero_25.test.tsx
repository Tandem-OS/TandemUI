...
describe('Hero_25 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_25.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_25.meta'));
    });
