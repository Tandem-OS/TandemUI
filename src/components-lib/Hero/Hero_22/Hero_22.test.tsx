...
describe('Hero_22 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_22.tsx');
  ...
  it('should use meta.tokens throughout the component', () => {
    ...
    expect(tokenUsage).toBeGreaterThanOrEqual(12); // Newsletter requires more tokens
  });
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_22.meta'));
    });

    expect(invalidImports).toHaveLength(0);
  });

  // Newsletter-specific test
  it('should have newsletter form functionality', () => {
    const { container } = render(
      <Hero_22
        newsletterPlaceholder="Enter your email"
        newsletterButtonText="Subscribe"
        newsletterMessage="Terms apply"
      />
    );
