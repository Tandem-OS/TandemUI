...
// Main Hero_18 component
const Hero_18: React.FC<Hero_18Props> = (props = {}) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        if (!validation.valid || validation.warnings) {
            console.group('Hero_18 Validation');
    ...
    return (
        <section
            ...
            role="banner"
            aria-label={meta.accessibility.screenReaderHints?.section || "Main hero content with video background, paragraph positioned left bottom"}
            id={uniqueId}
        >
            {/* Background Video */}
            <div
            ...
            <div className={`${meta.tokens.layout.contentContainer} ${meta.tokens.spacing.sectionY}`}>
                <div className={meta.tokens.layout.grid}>
                    ...
                    <div
                        className={meta.tokens.layout.leftColumn}
                    ...
                    <MotionWrapper
                        className={`${meta.tokens.layout.rightColumn} ${meta.tokens.spacing.rightPadding}`}
                        {...(animated ? getAnimationProps(fadeInUp, 0, 0.3, true) : {})}
                        data-testid="hero-right-column"
                    >
                        <MotionWrapper
                            className={`${meta.tokens.layout.rightContent} ${meta.tokens.spacing.rightContentOffset}`}
