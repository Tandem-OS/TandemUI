...
describe('Hero_18 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_18.tsx');
  ...
  it('should not contain hardcoded Tailwind classes', () => {
    ...
    if (violations.length > 0) {
      console.log('\n⚠️  Potential hardcoded classes found:');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => 
      imp.includes('tokens/') || imp.includes('design-system/') || (imp.includes('meta') && !imp.includes('./Hero_18.meta'))
    );
  ...
  it('should have proper video background hero structure with bottom/center positioning', () => {
    const { container } = render(
      <Hero_18
        title="Test Title"
        description="Test Description"
  ...
  it('should have correct layout structure for paragraph left bottom, heading & CTA right center', () => {
    const { container } = render(
      <Hero_18
        title="Test Title"
        description="Test Description"
    ...
    // Check for left column with paragraph
    expect(container.querySelector('[data-testid="hero-left-column"]')).toBeTruthy();
    // Check for right column with heading & CTAs
