// Hero_02.meta.ts
import { type Hero_02Meta } from './Hero_02.types';

const meta: Hero_02Meta = {
  version: "1.1",
  ...
  category: "hero",
  intent: "convert",
  layout_structure: "split-reverse", // Key difference: image left, content right

  // === Tokens ===
  tokens: {
    // Layout Structure - Image Left, Content Right (reverse of Hero_01)
    layout: {
      section: "relative w-full lg:h-screen lg:overflow-hidden",
      grid: "grid grid-cols-1 lg:grid-cols-2 lg:h-screen",
      contentColumn: "flex flex-col justify-center order-1 lg:order-2 min-h-[50vh] lg:min-h-0", // order-2 for large screens
      imageColumn: "relative h-96 lg:h-screen order-2 lg:order-1", // order-1 for large screens
  ...
  layoutContract: {
    ...
    breakpoints: {
      mobile: "single-col",
      tablet: "split-reverse",
      desktop: "split-reverse"
    }
  },
  ...
  accessibility: {
    landmarks: ["banner"],
    ariaHints: ["h1 first", "CTA prominently placed", "Image positioned left"],
    colorContrastMin: 4.5,
    focusManagement: "linear",
    keyboardNavigation: true,
    screenReaderHints: {
      image: "decorative",
      section: "Main hero content with image on left"
    }
  },
  ...
  // === Semantic Profile ===
  semanticProfile: {
    primaryConcepts: ["conversion", "introduction", "value-proposition", "trust-building", "visual-first"],
    contentTypes: ["hero-messaging", "cta-copy", "brand-positioning"],
    embeddingWeights: { visual: 0.4, copy: 0.4, interaction: 0.2 }, // Higher visual weight for image-left layout
    contextualRelevance: ["landing-page-entry", "product-introduction", "conversion-funnel-top", "visual-storytelling"]
  },

  // === Figma Export ===
  figma: {
    autoLayout: true,
    figmaComponentName: "Hero/02 - Split Image Left",
    componentSets: {
  ...
  code: {
    ...
    imports: [
      ...
      "import { type Hero_02Props, type ColorOverrides } from './Hero_02.types';",
      "import { validateHero02Props, sanitizeProps, formatValidationMessage } from './Hero_02.validators';",
      "import meta from './Hero_02.meta';"
    ],
    template: `const Hero_02: React.FC<Hero_02Props> = (props = {}) => {
  ...
  development: {
    ...
    documentation: {
      designNotes: "Primary conversion component for landing pages with reverse split layout (image left, content right). All styles are tokenized using native HTML elements. Fully independent with no component dependencies.",
      usageGuidelines: "Use native h1, p, button/a elements with complete token classes. Colors are fully customizable via props. Animations can be disabled for accessibility. No external component dependencies. Image appears on left side on desktop."
    }
  }
};
