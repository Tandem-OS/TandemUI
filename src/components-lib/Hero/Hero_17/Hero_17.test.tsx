...
describe('Hero_17 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_17.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => 
      imp.includes('tokens/') || imp.includes('design-system/') || (imp.includes('meta') && !imp.includes('./Hero_17.meta'))
    );
  ...
  it('should have proper video background hero structure with center/bottom positioning', () => {
    const { container } = render(
      <Hero_17
        title="Test Title"
        description="Test Description"
  ...
  it('should have correct layout structure for heading & CTA left center, paragraph right bottom', () => {
    const { container } = render(
      <Hero_17
        title="Test Title"
        description="Test Description"
