...
describe('Hero_10 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_10.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_10.meta'));
    });
  ...
  it('should have proper responsive swap structure', () => {
    const { container } = render(
      <Hero_10
        title="Test Title"
        description="Test Description"
    ...
    // Check for responsive swap specific elements
    expect(container.querySelector('[data-testid="hero-mobile-heading"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="hero-desktop-content"]')).toBeTruthy();
    expect(container.querySelector('[data-testid="hero-mobile-content"]')).toBeTruthy();
