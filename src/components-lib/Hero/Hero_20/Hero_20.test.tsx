...
describe('Hero_20 - Token Usage Validation', () => {
  let sourceCode: string;

  beforeAll(() => {
    const componentPath = path.join(__dirname, 'Hero_20.tsx');
  ...
  it('should only import from local meta file', () => {
    ...
    const invalidImports = importStatements.filter(imp => {
      return imp.includes('tokens/') ||
        ...
        (imp.includes('meta') && !imp.includes('./Hero_20.meta'));
    });
  ...
  it('should include newsletter form in rendered output', () => {
    ...
    // Check newsletter form elements exist
    expect(getByRole('complementary', { name: /newsletter signup form/i })).toBeInTheDocument();
    expect(getByPlaceholderText('Enter your email')).toBeInTheDocument();
    expect(getByRole('button', { name: /subscribe to newsletter/i })).toBeInTheDocument();
  });

  it('should have proper accessibility attributes for newsletter', () => {
    ...
    // Check accessibility
    const emailInput = getByLabelText(/email address for newsletter/i);
