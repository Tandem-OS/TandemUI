...
// Component styles hook - following Hero_19 exact pattern
const useComponentStyles = (
    ...
    return useMemo(() => {
        ...
        const getNewsletterVars = (
            ...
            return {
                [`--newsletter-${prefix}-bg`]: getColor(newsletter.input?.background, defaultNewsletter.input.background),
                [`--newsletter-${prefix}-text`]: getColor(newsletter.input?.text, defaultNewsletter.input.text),
                [`--newsletter-${prefix}-border`]: getColor(newsletter.input?.border, defaultNewsletter.input.border),
                [`--newsletter-${prefix}-focus-border`]: getColor(newsletter.input?.focusBorder, defaultNewsletter.input.focusBorder),
                [`--newsletter-${prefix}-placeholder`]: getColor(newsletter.input?.placeholder, defaultNewsletter.input.placeholder),
        ...
        return {
            ...
            ...getNewsletterVars(userColors?.newsletter, defaults.newsletter, 'input'),
        };
    }, [userColors, theme]);
};

// Styled components - ONLY for hover and focus states (following Hero_19 pattern)
...
// Newsletter component (replacing ActionButton from Hero_19)
...
// Main Hero_20 component - following Hero_19 exact structure
const Hero_20: React.FC<Hero_20Props> = (props = {}) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        if (!validation.valid || validation.warnings) {
            console.group('Hero_20 Validation');
    ...
    return (
        <section
            ...
            role="banner"
            aria-label={meta.accessibility.screenReaderHints?.section || "Main hero content with centered text and newsletter signup above full-width image"}
            id={uniqueId}
        >
            {/* Content Section */}
            <div
