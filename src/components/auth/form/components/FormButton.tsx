...
import { type FC, type ButtonHTMLAttributes, type CSSProperties, useCallback, memo } from 'react'
  ...
  'inline-flex items-center justify-center font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed relative rounded-lg flex items-center gap-sm',
  {
    variants: {
      variant: {
        ...
        basic: 'border-2 transition-all duration-200 cursor-pointer',
      },
      size: {
...
const FormButton: FC<FormButtonProps> = memo(({
  variant = 'solid',
  size = 'md',
  fullWidth = false,
  className,
