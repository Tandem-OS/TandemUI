...
import { swiperData } from '@/lib/requests/SwiperRequest';
    ...
    if (showRoundCompletion && !kingOfHill.isActive) {
      ...
      if (roundChoices.length > 0) {
        ...
        const payload = {
          ...
          saved: roundSummary.saved,
          rejected: roundSummary.rejected,
          completion_time: roundSummary.completion_time,
          gesture_vs_button_ratio: roundSummary.gesture_vs_button_ratio,
          total_hesitation_ms: roundSummary.total_hesitation_ms,
          save_count: roundSummary.save_count,
          superlike_count: roundSummary.superlike_count,
          average_view_duration_ms: roundSummary.average_view_duration_ms ?? 0,
          total_swipes: breakdown.total_swipes,
          liked_count: breakdown.liked,
          saved_count: breakdown.saved,
        ...
        const sendData = async () => {
          try {
            await swiperData(payload);

            // then continue with your checkRoundWithBackend logic
