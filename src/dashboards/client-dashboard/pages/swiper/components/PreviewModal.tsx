...
import PreviewFrame from './PreviewFrame'; // <-- STEP 1: Naya component import karein
...
const getRoundComponents = (roundsCompleted: number) => {
    ...
    if (roundsCompleted >= 8 || components.length > 3) {
            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className={`bg-background-primary flex flex-col ${isFullscreen ? 'w-full h-full' : 'rounded-2xl shadow-2xl w-full h-full max-w-7xl max-h-[90vh]'
                            }`}
                    >
                        {/* Header */}
                                ...
                                <div className="hidden sm:flex items-center bg-background-secondary rounded-xl p-xs border border-border-default">
                                    {(Object.entries(viewportSizes) as [ViewportSize, ViewportConfig][]).map(([key, config]) => (
                                        <button
                                            ...
                                            className={`relative px-sm py-xs lg:px-md lg:py-sm rounded-lg transition-all duration-200 ${viewport === key
                                                    ? 'text-text-primary'
                            ...
                            <div className="flex items-center justify-center space-x-xs">
                                {(Object.entries(viewportSizes) as [ViewportSize, ViewportConfig][]).map(([key, config]) => (
                                    <button
                                        key={key}
                                        onClick={() => setViewport(key)}
                                        className={`p-sm rounded-lg transition-all ${viewport === key
                        ...
                        <div className="flex-1 bg-background-muted overflow-hidden">
                            <div className="h-full flex items-center justify-center">
                                ...
                                >
                                    <PreviewFrame>
                                        <div className="h-full overflow-y-auto preview-container bg-white">
                        ...
                        <div className="flex items-center justify-center px-md sm:px-lg lg:px-xl py-sm  border-t border-border-default bg-background-primary">
                            <motion.button
                                onClick={handleClose}
